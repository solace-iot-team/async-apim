...
import { E_CALL_STATE_ACTIONS, E_COMPONENT_STATE_ADD_USER, ManageUsersCommon, TManagedObjectId, TViewManagedObject } from "./ManageOrganizationUsersCommon";
...
export const AddOrganizationUser: React.FC<IAddOrganizationUserProps> = (props: IAddOrganizationUserProps) => {
  ...
  const apiGetManagedObjectListPage = async(pageSize: number, pageNumber: number, sortFieldName: string, sortDirection: EAPSSortDirection, searchWordList?: string): Promise<TApiCallState> => {
    ...
    try { 
      const listApsUsersResponse: ListApsUsersResponse = await ApsUsersService.listApsUsers({
        ...
        excludeSearchOrganizationId: props.organizationEntityId.id,
  ...
  const apiAddManagedObjectToOrgId = async(mo: TManagedObject): Promise<TApiCallState> => {
    ...
    const logName = `${componentName}.${funcName}()`;
    let callState: TApiCallState = ApiCallState.getInitialCallState(E_CALL_STATE_ACTIONS.API_ADD_USER_TO_ORG, `add user ${mo.apiObject.userId} to org ${props.organizationEntityId.displayName}`);
    try {
  ...
  React.useEffect(() => {
    ...
    if (apiCallStatus !== null) {
      if(!apiCallStatus.success) props.onError(apiCallStatus);
        ...
        props.onSuccess(apiCallStatus, { id: selectedManagedObject.id, displayName: selectedManagedObject.displayName });
      }
    }
