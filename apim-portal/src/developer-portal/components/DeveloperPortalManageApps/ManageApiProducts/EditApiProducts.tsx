...
export const EditApiProducts: React.FC<IEditApiProductsProps> = (props: IEditApiProductsProps) => {
  ...
  const FormId = `DeveloperPortalManageApps_ManageApiProducts_${ComponentName}`;
  ...
  const hasApiProductListChanged = (): boolean => {
    ...
    managedObject.forEach( (moElem: TManagedObjectElement) => {
      const exists = props.apDeveloperPortalAppDisplay.apAppApiProductDisplayList.find( (x) => {
  ...
  const apiUpdateManagedObject = async(mo: TManagedObject): Promise<TApiCallState> => {
    ...
    const logName = `${ComponentName}.${funcName}()`;
    let callState: TApiCallState = ApiCallState.getInitialCallState(E_CALL_STATE_ACTIONS.API_UPDATE_APP_API_PRODUCTS, `update api products for app: ${props.apDeveloperPortalAppDisplay.apEntityId.displayName}`);
    try {
      switch(props.appType) {
        ...
        case EAppType.TEAM:
          ...
          await APDeveloperPortalTeamAppsDisplayService.apiUpdate_ApDeveloperPortalAppDisplay_AppApiProductDisplayList({
            organizationId: props.organizationId,
            apDeveloperPortalAppDisplay: props.apDeveloperPortalAppDisplay,
            apDeveloperPortalAppApiProductDisplayList: mo
          });
          break;
        default:
  ...
  const apiAddApiProductToApp = async(apiProductEntityId: TAPEntityId): Promise<TApiCallState> => {
    ...
    try {
      ...
      const existing: TAPDeveloperPortalAppApiProductDisplay | undefined = props.apDeveloperPortalAppDisplay.apAppApiProductDisplayList.find((x) => {
  ...
  const doInitialize = async () => {
    // work on a copy 
    setManagedObject(JSON.parse(JSON.stringify(props.apDeveloperPortalAppDisplay.apAppApiProductDisplayList)));
  }
