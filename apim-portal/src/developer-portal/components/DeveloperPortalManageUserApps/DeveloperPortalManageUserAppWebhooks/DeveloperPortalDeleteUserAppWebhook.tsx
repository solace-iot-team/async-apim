...
import { AppPatch, AppsService, WebHook } from "@solace-iot-team/apim-connector-openapi-browser";
import { APClientConnectorOpenApi } from "../../../../utils/APClientConnectorOpenApi";
import { APManagedWebhook, TAPManagedWebhook, TAPManagedWebhookList, TAPOrganizationId, TAPViewManagedWebhook } from "../../../../components/APComponentsCommon";
...
export interface IDeveloperPortalDeleteUserAppWebhookProps {
  ...
  deleteManagedWebhook: TAPViewManagedWebhook;
...
export const DeveloperPortalDeleteUserAppWebhook: React.FC<IDeveloperPortalDeleteUserAppWebhookProps> = (props: IDeveloperPortalDeleteUserAppWebhookProps) => {
  ...
  const DeleteManagedObjectConfirmDialogHeader = `Confirm Deleting Webhook for App: ${props.viewManagedAppWebhookList.appDisplayName}`;
  ...
  const apiUpdateManagedObject = async(): Promise<TApiCallState> => {
    ...
    if(!props.viewManagedAppWebhookList.apiAppResponse.webHooks) throw new Error(`${logName}: props.viewManagedAppWebhookList.apiAppResponse.webHooks is undefined`);
    ...
    const webhookIndexToDelete: number = existigManagedWebhookList.findIndex( (mwh: TAPManagedWebhook) => {
      return (mwh.environment.name === deleteManagedWebhookList[0].environment.name);
    });
    ...
    const apiAppPatch: AppPatch = APManagedWebhook.createApiAppUpdateRequestBodyFromManagedWebhookList(props.viewManagedAppWebhookList.apiAppResponse, newManagedWebhookList);
  ...
  const renderDeleteManagedObjectDialogContent = (): JSX.Element => {
    return (
      <React.Fragment>
        <p>TODO: show the correct reference (webhook for env)</p>
        <p>Deleting Webhook for App: <b>{props.viewManagedAppWebhookList.appDisplayName}</b>.</p>
