...
export const APDisplayUserOrganizationRoles: React.FC<IAPDisplayUserOrganizationRolesProps> = (props: IAPDisplayUserOrganizationRolesProps) => {
  ...
  const renderOrgRoleList = (memberOfOrganizations: APSOrganizationRolesResponseList): JSX.Element => {
    ...
    const _sortable: boolean = memberOfOrganizations.length > 1;
    return (
      <div className="card">
        <DataTable
          ...
          scrollable 
        >
          {/* <Column header="Id" field="organizationId" sortable={_sortable} /> */}
          <Column header="Organization" field="organizationDisplayName" sortable={_sortable} />
  ...
  const renderAllOrgs = (memberOfOrganizations: APSOrganizationRolesResponseList): JSX.Element => {
  ...
  const renderSingleOrg = (apsOrganizationRoles: APSOrganizationRolesResponse): JSX.Element => {
    return (
      <React.Fragment>
  ...
  const renderComponent = (apsUserResponse: APSUserResponse, organizationId: CommonName | undefined): JSX.Element => {
    const funcName = 'renderComponent';
    const logName = `${componentName}.${funcName}()`;
    
    if(apsUserResponse.memberOfOrganizations === undefined) return (
      <React.Fragment>
        <p>NOT A MEMBER OF ANY ORGANIZATION</p>
      </React.Fragment>
    );

    if(organizationId === undefined) return renderAllOrgs(apsUserResponse.memberOfOrganizations);

    const apsOrganizationRoles: APSOrganizationRolesResponse | undefined = apsUserResponse.memberOfOrganizations.find((apsOrganizationRoles: APSOrganizationRolesResponse) => {
  ...
  return (
    <div className={props.className ? props.className : 'card'}>
      { renderComponent(props.apsUserResponse, props.organizationId) }
    </div>
  );
}
